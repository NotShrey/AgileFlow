...
//useLoginMutation: A custom hook generated by Redux Toolkit Query for the login mutation.
//setCredentials: An action from the auth slice for setting user credentials in the Redux state. 
...
const Login = () => {
  ...
  //submitHandler: An async function that handles form submission.
// login(data): Triggers the login mutation with the form data.
// unwrap(): Extracts the response or throws an error.
// dispatch(setCredentials(result)): Saves the user credentials in the Redux state. ****important***
// navigate("/dashboard"): Redirects the user to the dashboard upon successful login.
// toast.error(): Displays an error message if the login fails.

  useEffect(() => {
    user && navigate("/dashboard"); // if it exists
  }, [user]); //[dependency array] only refresh or reload when user is changed
  
  // useEffect: Runs the side effect to navigate to the dashboard if the user is already logged in.
  // [user]: The dependency array ensures this effect runs only when the user state changes.
