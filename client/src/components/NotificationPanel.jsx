      ...
      <Popover className="relative">
        ...
        >
          <Popover.Panel className="absolute -right-16 md:-right-2 z-10 mt-5 flex w-screen max-w-max  px-4">
            {({ close }) =>
              data?.length > 0 && ( // checking if the data exists or not
                <div className="w-screen max-w-md flex-auto overflow-hidden rounded-3xl bg-white text-sm leading-6 shadow-lg ring-1 ring-gray-900/5">
                  <div className="p-4">
                    {/* if we have more notification then we are just showing the recent 5 notis here*/}
                    {data?.slice(0, 5).map((item, index) => (
                      ...
                      >
                        <div className="mt-1 h-8 w-8 flex items-center justify-center rounded-lg bg-gray-200 group-hover:bg-white">
                          {ICONS[item.notiType]}
                          {/* the icon will be decided acc to the notification type */}
                        </div>
                        ...
                        >
                          <div className="flex items-center gap-3 font-semibold text-gray-900 capitalize">
                            ...
                            <span className="text-xs font-normal lowercase">
                              ...
                              {/* for knowing when we go the notification data and time */}
                            </span>
                          </div>
                          <p className="line-clamp-1 mt-1 text-gray-600"> 
                          {/* for a single line notification */}
                            {item.text}
                          </p>
                        </div>
                      </div>
